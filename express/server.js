...
// blockchainService 內部會讀取 .env 的 BLOCKCHAIN_RPC_URL, PRIVATE_KEY, CONTRACT_ADDRESS 等
// 啟動時會自動建立 Web3 與 Contract 實例

const authController = require('./controllers/authController');

// === 路由 ===
const authRouter = require('./routes/auth');     // 若使用 /auth prefix + authController
const authRoutes = require('./routes/authRoutes');
const uploadRoutes = require('./routes/uploadRoutes');
const trademarkRoutes = require('./routes/trademarkRoutes');
const paymentRoutes = require('./routes/paymentRoutes');
// (可再加入 userRoutes, infringementRoutes 等)
...
// 1) 使用 /auth prefix 時，導向 authRouter => /auth/register & /auth/login
...
// (注意：這可能會跟上面 authRouter 重複路徑，請確認需求)
