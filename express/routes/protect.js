/**
 * express/routes/protect.js (整合優化版)
 *
 * 【本次整合優化】:
 * - 綜合「最終修正版」與「已整合修正」兩份程式碼的優點。
 * - [BUG修復] 在 /scan/:fileId 路由中，從資料庫的 mimetype 重新推導檔案副檔名，解決因副檔名不匹配導致的檔案找不到 (404) 問題。 (採納自「最終修正版」的核心邏輯)
 * - [邏輯保留] 維持 /step1 路由中使用 `indexImageVector` 進行向量索引的邏輯，以配合更新後的 `vectorSearch` 服務。 (源自「已整合修正」)
 * - [邏輯保留] 維持 /scan/:fileId 路由中處理 Python 服務回傳的 `{id, score}` 格式的向量搜尋結果。 (源自「已整合修正」)
 * - [BUG修復確認] /step1 中呼叫 `generateCertificatePDF` 時已包含 `pdfPath` 參數，此問題在「已整合修正」版中已解決。
 *
 * 【核心功能】:
...
        // ★★★★★ 【核心BUG修復】 ★★★★★
        // 從資料庫中的 mimetype 推導檔案的正確副檔名，解決因上傳時檔名與類型不符，導致此處找不到檔案的問題。
        // 此邏輯與 /step1 的副檔名修正邏輯保持一致。
        let correctedExt = '';
...
        else correctedExt = path.extname(fileRec.filename) || ''; // Fallback
