...
router.get('/scan/:fileId', async (req, res)=>{
  try {
    ...
    // 如果是影片，且長度 <= 30 秒，就抽幀再跑 doMultiReverseImage
    let isVideo = false;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (fs.existsSync(notoTC)) {
        ...
        console.warn('[generatePdf] NotoSansTC 字體不存在，改用 Helvetica');
        doc.font('Helvetica');
      }
