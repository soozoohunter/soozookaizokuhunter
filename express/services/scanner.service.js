// express/services/scanner.service.js (New Logic with Better Logging)
...
async function performFullScan(options) {
  ...
  const tineyeLinks = tineyeResult.success
    ? tineyeResult.matches.flatMap((m) => (m.backlinks.length > 0 ? m.backlinks : [m.url]))
    : [];
  ...
  logger.info(
    `[Scanner Service] Full scan completed in ${duration}ms. Found ${verifiedMatches.length} verified matches.`,
  );
