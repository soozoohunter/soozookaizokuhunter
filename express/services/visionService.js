...
// --- [診斷程式碼開始] ---
logger.info("==============================================");
logger.info("DIAGNOSTIC LOG: Checking Environment Variable...");
logger.info(`Value of GOOGLE_APPLICATION_CREDENTIALS is: "${process.env.GOOGLE_APPLICATION_CREDENTIALS}"`);
logger.info("==============================================");
// --- [診斷程式碼結束] ---
...
async function infringementScan({ buffer, keyword }) {
    ...
    if (!keyword) logger.warn('[Service] Keyword not provided; RapidAPI searches will be skipped.');

    logger.info('[Service] Starting infringement scan...');
    ...
    try {
        ...
        logger.info(`[Service] Temp file created for TinEye scan: ${tmpPath}`);
        tmpFileCreated = true;

        logger.info('[Service] Calling TinEye API...');
        const start = Date.now();
        const result = await tinEyeApi.searchByFile(tmpPath);
        tineyeResult = { success: true, links: result.links || [], error: null };
        logger.info(`[Service] TinEye scan completed in ${Date.now() - start}ms, found ${tineyeResult.links.length} links.`);
    } catch (err) {
        logger.error('[Service] TinEye API process failed:', err.message);
    ...
    try {
        logger.info('[Service] Calling Google Vision API...');
        const start = Date.now();
        ...
        logger.info(`[Service] Google Vision scan completed in ${Date.now() - start}ms, found ${urls.length} links.`);
    } catch (err) {
        logger.error('[Service] Google Vision API call failed:', err.message);
        if (err.code === 16) {
            logger.error('[Service] FATAL: Google Vision Authentication failed! Verify key file, IAM role, and that the GOOGLE_APPLICATION_CREDENTIALS environment variable is correctly set inside the container.');
        }
    ...
    if (process.env.RAPIDAPI_KEY && keyword) {
        logger.info(`[Service] Calling RapidAPI integrations with keyword: "${keyword}"`);
        ...
        logger.warn(`[Service] Skipping RapidAPI integrations. Reason: ${reason}.`);
    }
    ...
    if (tmpFileCreated) {
        try {
            ...
            logger.info(`[Service] Deleted temp file: ${tmpPath}`);
        } catch (e) {
            logger.error(`[Service] Failed to delete temp file: ${e.message}`);
        }
    }

    logger.info(`[Service] Overall scan finished in ${Date.now() - overallStart}ms.`);
